...
<div class="max-w-lg">
    <form method="POST" action="{{ isset($category) ? route('admin.service-categories.update', $category) : route('admin.service-categories.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $category->name ?? '') }}" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">
            @error('name') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block text-sm font-medium text-charcoal mb-1">Ikon</label>
            <input type="text" name="icon" value="{{ old('icon', $category->icon ?? '') }}" placeholder="contoh: ✂️ atau nama ikon" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">
        ...
        <div>
            ...
            <input type="number" name="order" value="{{ old('order', $category->order ?? 0) }}" min="0" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">
        </div>
