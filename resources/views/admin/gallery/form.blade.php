...
<div class="max-w-lg">
    <form method="POST" action="{{ isset($gallery) ? route('admin.gallery.update', $gallery) : route('admin.gallery.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $gallery->title ?? '') }}" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">
        ...
        <div>
            ...
            @endif
            <input type="file" name="image" accept="image/*" {{ isset($gallery) ? '' : 'required' }} class="w-full text-sm text-dark-gray file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-rose-gold/10 file:text-rose-gold">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $gallery->order ?? 0) }}" min="0" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">
            </div>
