...
<div class="max-w-lg">
    <form method="POST" action="{{ isset($testimonial) ? route('admin.testimonials.update', $testimonial) : route('admin.testimonials.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $testimonial->name ?? '') }}" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-#C0001E/30">
        ...
        <div>
            ...
            @if(isset($testimonial) && $testimonial->photo)
                <img src="{{ asset('media/' . $testimonial->photo) }}" class="w-20 h-20 object-cover rounded-full mb-2" alt="">
            @endif
            <input type="file" name="photo" accept="image/*" class="w-full text-sm text-dark-gray file:mr-3 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-#C0001E/10 file:text-#C0001E">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="rating" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-#C0001E/30">
                    @for($i = 5; $i >= 1; $i--)
        ...
        <div>
            ...
            <select name="program_id" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-#C0001E/30">
        ...
        <div>
            ...
            <textarea name="content" rows="4" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-#C0001E/30">{{ old('content', $testimonial->content ?? '') }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="order" value="{{ old('order', $testimonial->order ?? 0) }}" min="0" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-#C0001E/30">
            </div>
