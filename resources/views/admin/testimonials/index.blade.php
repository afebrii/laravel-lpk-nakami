...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <a href="{{ route('admin.testimonials.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-#C0001E text-white text-sm font-semibold rounded-xl hover:bg-#C0001E-dark transition-colors">
                    ...
                    <tr class="hover:bg-gray-50/50">
                        <td class="px-4 py-3">
                            <div class="w-10 h-10 bg-gray-100 rounded-full overflow-hidden">
                                @if($item->photo)
                                    <img src="{{ asset('media/' . $item->photo) }}" class="w-full h-full object-cover" alt="">
                                @else
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button type="button" 
                                        @click="$dispatch('open-delete-modal', { url: '{{ route('admin.testimonials.destroy', $item) }}', message: 'Hapus testimoni {{ addslashes($item->name) }} ini?' })"
