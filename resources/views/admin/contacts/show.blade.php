...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.contacts.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-dark-gray hover:text-#C0001E transition-colors">
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
        ...
        <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
            <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 rounded-full bg-#C0001E text-white flex items-center justify-center font-bold text-lg shrink-0 shadow-sm">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3 text-sm text-gray-500 mt-0.5">
                            <a href="mailto:{{ $contact->email }}" class="hover:text-#C0001E transition-colors">{{ $contact->email }}</a>
        ...
        <div class="px-6 py-5 bg-gray-50/50 border-t border-gray-100 mt-4 flex gap-3">
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $contact->phone) }}?text={{ urlencode('Halo ' . $contact->name . ', ini dari Admin LPK Nakami membalas pesan Anda mengenai \"' . $contact->subject . '\".') }}" target="_blank" class="inline-flex justify-center items-center gap-2 px-5 py-2.5 bg-[#25D366] text-white text-sm font-semibold rounded-xl hover:bg-[#1fae54] transition-colors shadow-sm w-full sm:w-auto">
