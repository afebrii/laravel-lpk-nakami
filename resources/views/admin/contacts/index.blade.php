                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors block sm:table-row {{ !$contact->is_read ? 'bg-rose-gold/5 font-semibold text-charcoal' : 'text-dark-gray' }}">
                        ...
                        <td class="px-6 py-4 text-left sm:text-right block sm:table-cell">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.contacts.destroy', $contact) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus pesan ini secara permanen?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus Permanen">
