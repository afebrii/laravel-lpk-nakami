...
<div class="max-w-lg">
    <form method="POST" action="{{ isset($category) ? route('admin.program-categories.update', $category) : route('admin.program-categories.store') }}" class="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-rose-gold/30">{{ old('description', $category->description ?? '') }}</textarea>
        </div>
